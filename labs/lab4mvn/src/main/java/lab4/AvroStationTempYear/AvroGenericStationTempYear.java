...
public class AvroGenericStationTempYear extends Configured implements Tool
{
	...
	public static class AvroMapper extends Mapper<LongWritable, Text, AvroKey<GenericRecord>, NullWritable>
	{
		...
		protected void setup(Mapper<LongWritable, Text, AvroKey<GenericRecord>, NullWritable>.Context context)
			...
			// URI schemaFile = DistributedCache.getCacheFiles(context.getConfiguration())[0];
	...
	public int run(String[] args) throws Exception
	{
		...
		Path schemaFile = new Path(args[2]);
		SCHEMA = new Schema.Parser().parse(hdfs.open(schemaFile));
		job.addCacheFile(new URI(args[2] + "#customSchema"));
