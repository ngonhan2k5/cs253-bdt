...
public abstract class SFileOutputFormat<K, V> extends OutputFormat<K, V> {


    public static final Log logger = LogFactory.getLog(AvgTempE2.ATMapper.class);
    /**
    ...
    public static void setCompressOutput(Job job, boolean compress) {
        job.getConfiguration().setBoolean(SFileOutputFormat.COMPRESS, compress);
    }
    ...
    public static boolean getCompressOutput(JobContext job) {
        return job.getConfiguration().getBoolean(SFileOutputFormat.COMPRESS, false);
    }
     ...
     * 
     * @param job        the job to modify
     * @param codecClass the {@link CompressionCodec} to be used to compress the job
     *                   outputs
     */
    public static void setOutputCompressorClass(Job job, Class<? extends CompressionCodec> codecClass) {
        setCompressOutput(job, true);
        job.getConfiguration().setClass(SFileOutputFormat.COMPRESS_CODEC, codecClass, CompressionCodec.class);
    }
    ...
    public static Class<? extends CompressionCodec> getOutputCompressorClass(JobContext job,
        ...
        String name = conf.get(SFileOutputFormat.COMPRESS_CODEC);
        if (name != null) {
            try {
     ...
     *
     * @param job       The job to modify
     * @param outputDir the {@link Path} of the output directory for the map-reduce
     *                  job.
     */
    public static void setOutputPath(Job job, Path outputDir) {
        ...
        job.getConfiguration().set(SFileOutputFormat.OUTDIR, outputDir.toString());
    }
    ...
    public static Path getOutputPath(JobContext job) {
        String name = job.getConfiguration().get(SFileOutputFormat.OUTDIR);
     ...
     * 
     * <p>
     * Some applications need to create/write-to side-files, which differ from the
     * actual job-outputs.
     * 
     * <p>
     * In such cases there could be issues with 2 instances of the same TIP (running
     * simultaneously e.g. speculative tasks) trying to open/write-to the same file
     * (path) on HDFS. Hence the application-writer will have to pick unique names
     * per task-attempt (e.g. using the attemptid, say
     * <tt>attempt_200709221812_0001_m_000000_0</tt>), not just per TIP.
     * </p>
     * 
     * <p>
     * To get around this the Map-Reduce framework helps the application-writer out
     * by maintaining a special
     * <tt>${mapreduce.output.fileoutputformat.outputdir}/_temporary/_${taskid}</tt>
     * sub-directory for each task-attempt on HDFS where the output of the
     * task-attempt goes. On successful completion of the task-attempt the files in
     * the
     * <tt>${mapreduce.output.fileoutputformat.outputdir}/_temporary/_${taskid}</tt>
     ...
     * required in a work directory during execution of his task i.e. via
     * {@link #getWorkOutputPath(TaskInputOutputContext)}, and the framework will
     * move them out similarly - thus she doesn't have to pick unique paths per
     * task-attempt.
     * </p>
     * 
     * <p>
    ...
    /**
     * Helper function to generate a {@link Path} for a file that is unique for the
     * task within the job output directory.
     *
     * <p>
     * The path can be used to create custom files from within the map and reduce
     * tasks. The path name will be unique for each task. The path parent will be
    ...
    public Path getDefaultWorkFile(TaskAttemptContext context, String extension) throws IOException {
        ...
        logger.info("======================"+getUniqueFile(context, getOutputName(context), extension));
        System.out.println("======================"+getUniqueFile(context, getOutputName(context), extension));
